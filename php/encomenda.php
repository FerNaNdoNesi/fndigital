	...
	$Arq = fopen("../block/sn.txt", 'rb');
		...
		if(!($conect = mysql_select_db($db,$conexao))) { 
		   ...
		   exit; 
			...
			$sql =  "SELECT * FROM produtos WHERE codigoProduto='".$_GET['codProduto']."'";
			$res = mysql_query($sql);
			...
			$sql =  "INSERT INTO encomendas (codigo, nome, telefone, email, endereco, cidade, produto, preco, data) 
			VALUES (NULL, '".htmlspecialchars($_POST['nome'])."', '".htmlspecialchars($_POST['telefone'])."', '".htmlspecialchars($_POST['email'])."', '".htmlspecialchars($_POST['endereco'])."', '".htmlspecialchars($_POST['cidade'])."',
			 '".$nomeProduto."', '".$precoProduto."', NULL);";
			$res = mysql_query($sql);
			$prod = $_GET['codProduto'];
			header("Location: ../".$_GET['link']."?codProduto=".$prod."&popUpMsg=encomendar");				
?>